...
### Fixed
- Concurrent downloads no longer publish each other's half-written file. `download_all` fans records out over threads, and records share destinations: every record of a dataset resolves the same root metadata, and every run of a subject the same `_electrodes.tsv` and `_coordsystem.json`. `nemar.download_one` stages every transfer at a fixed `<dest>.part`, so callers sharing a destination opened the same staging file, truncated each other's bytes, and whichever finished first renamed onto the destination whatever the staging file held at that instant — routinely nothing. The losers then failed their own rename with `FileNotFoundError`, swallowed as a warning, and the `if dest.exists()` guards latched the truncated file for every later run. On `nm000183` a clean single-subject download landed a 0-byte `participants.tsv` against the 441 bytes the manifest declares, and a full run left 5 of 14 `_electrodes.tsv` empty, which surfaces downstream as `KeyError: 'name'` from `mne_bids`. `_download_via_nemar` now stages each transfer under a name unique to the caller and publishes it with `os.replace`, so concurrent callers no longer share a staging file — across processes as well as threads, which matters because the SLURM example shares one `EEGDASH_CACHE_DIR` across 50 array tasks and a `threading.Lock` cannot span those. The per-destination lock is kept on top, where it now serves to deduplicate rather than to protect: 86 records of `nm000183` resolve one `sub-012_electrodes.tsv`, and only the first transfers it. Relatedly, `nemar.download_one` *returns* its verdict rather than raising, and discarding it published corruption as success — `commit()` reopens the staging file in append mode, which creates it, so a caller whose staging file a peer had already consumed wrote a 0-byte destination and reported OK. The result is now checked and a non-OK transfer discarded. Two writers outside this path (`S3Client.get_file`, and the inline-sidecar and GitHub-raw writes in `_resolve_one_nemar_entry`) still stage at a fixed name; that is a pre-existing defect and is left for a follow-up.
...
- Digestion enumerates the BIDS JSON sidecar of every datatype. `_DEP_SUFFIXES` listed `_eeg.json` and `_nirs.json` but neither `_ieeg.json` nor `_meg.json`, so no iEEG or MEG record carried its sidecar as a dependency and clients never fetched it — none of the 632 records of `nm000183` nor the 257 of `nm000182` list one, leaving `raw.info["line_freq"]` at `None` for a dataset whose sidecar declares `PowerLineFrequency: 50`, and the downloaded tree short of a file BIDS requires. Rather than extend the hand-maintained list (which had already drifted, and still lacked `_emg.json`), the sidecar name is now derived per record from that record's own BIDS suffix, so it cannot drift again and a record no longer picks up a co-recorded datatype's sidecar from a shared session directory. `dep_keys` is written at digest time, so already-digested datasets need a re-ingestion to pick the above up — and it must be `5_inject.py --force`. `ingestion_fingerprint` is computed from file paths and sizes only, so a plain re-run produces an identical fingerprint, `filter_changed_datasets` skips the dataset, and the rebuilt `dep_keys` is discarded.

## [0.9.1] - 2026-09-10

### Changed
- Tutorials use real EEGDash recordings and EEGPrep, with small downloads for documentation CI (#411).
- Updated the 2025 and 2026 challenge tutorials, including externalizing prediction, image retrieval, sleep onset, and EMG-to-pose (#412).

### Fixed
- Embedded EEGLAB recordings no longer request unnecessary FDT companions (#415).
- Fresh loaders resume missing NEMAR companions when the primary recording is already cached (#416).
- Preserve the BIDS recording entity when loading datasets (#412).
- Link the NeuroAI interoperability tutorial to the current pretraining example (#415).
