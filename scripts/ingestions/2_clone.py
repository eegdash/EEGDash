...
def get_git_clone_url(dataset: dict, source: str) -> str:
    ...
    if source == "openneuro":
        ...
        return dataset.get("clone_url") or f"https://github.com/nemardatasets/{dataset_id}"
    elif source == "gin":
...
def clone_git_shallow(
    ...
    if clone_dir.exists():
        ...
        if validate_bids:
            ...
            if not is_valid:
                return {"status": "invalid_bids", "dataset_id": dataset_id, "source": source, "reason": reason}
    ...
    if not url:
        return {"status": "error", "dataset_id": dataset_id, "source": source, "error": "No clone URL"}
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            return {"status": "failed", "dataset_id": dataset_id, "source": source, "error": result.stderr[:300]}
        ...
        if clone_dir.exists():
            shutil.rmtree(clone_dir, ignore_errors=True)
        return {"status": "timeout", "dataset_id": dataset_id, "source": source, "timeout": timeout}
        
    except Exception as e:
        if clone_dir.exists():
            shutil.rmtree(clone_dir, ignore_errors=True)
        return {"status": "error", "dataset_id": dataset_id, "source": source, "error": str(e)}
...
def fetch_gin_manifest(
    ...
    if not match:
        return {"status": "error", "dataset_id": dataset_id, "source": "gin", "error": "Cannot parse GIN URL"}
    ...
    try:
        ...
        if response.status_code != 200:
            return {"status": "error", "dataset_id": dataset_id, "source": "gin", "error": f"API error {response.status_code}"}
        ...
        if validate_bids:
            ...
            if not has_desc:
                return {"status": "invalid_bids", "dataset_id": dataset_id, "source": "gin", "reason": "No dataset_description.json"}
        ...
        return {"status": "error", "dataset_id": dataset_id, "source": "gin", "error": str(e)}
...
def fetch_figshare_manifest(
    ...
    try:
        ...
        if response.status_code != 200:
            return {"status": "error", "dataset_id": dataset_id, "source": "figshare", "error": f"API error {response.status_code}"}
        ...
        return {"status": "error", "dataset_id": dataset_id, "source": "figshare", "error": str(e)}
...
def fetch_zenodo_manifest(
    ...
    try:
        ...
        if response.status_code != 200:
            return {"status": "error", "dataset_id": dataset_id, "source": "zenodo", "error": f"API error {response.status_code}"}
        ...
        return {"status": "error", "dataset_id": dataset_id, "source": "zenodo", "error": str(e)}
...
def fetch_osf_manifest(
    ...
    try:
        ...
        return {"status": "error", "dataset_id": dataset_id, "source": "osf", "error": str(e)}
...
def is_bids_data_file(filename: str) -> bool:
    ...
    bids_extensions = {
        ".edf", ".bdf", ".set", ".fif", ".vhdr", ".eeg", ".vmrk",
        ".nii", ".nii.gz", ".meg4", ".ds",
        ".json", ".tsv",  # Sidecar files
    }
...
def peek_zip_contents(url: str, timeout: int = 30, max_bytes: int = 65536) -> list[str] | None:
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        choices=["openneuro", "nemar", "gin", "figshare", "zenodo", "osf", "scidb", "datarn"],
    ...
    print(f"Manifest only: {'yes' if args.manifest_only else 'no (git sources will clone)'}")
    print("=" * 60)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(
                process_dataset, ds, args.output, args.timeout, args.validate_bids, args.manifest_only
            ): ds
            for ds in datasets
        }
        ...
        with tqdm(total=len(datasets), desc="Processing") as pbar:
            for future in as_completed(futures):
                ...
                if result.get("status") in ("failed", "timeout", "error", "invalid_bids"):
    ...
    for result in results:
        if result.get("manifest"):
            enriched.append({
                ...
                "bids_file_count": len(result.get("manifest", {}).get("bids_files", [])),
            })
