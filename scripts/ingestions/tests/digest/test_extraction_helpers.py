"""Unit tests for the decomposition helpers in 3_digest.py."""
...
def test_readme_prefers_no_extension_over_md(tmp_path: Path):
    """``README`` is tried before ``README.md``; different extensions used to avoid APFS case-folding."""
    digest = load_digest()
...
def test_readme_tolerates_non_utf8(tmp_path: Path):
    """README with invalid UTF-8 yields None (function tries next candidate)."""
    digest = load_digest()
...
def test_demographics_ignores_invalid_ages(tmp_path: Path):
    """Ages outside (0, 120) and non-numeric values are skipped."""
    digest = load_digest()
    (tmp_path / "participants.tsv").write_text(
        "participant_id\tage\nsub-01\tn/a\nsub-02\t-1\nsub-03\t150\nsub-04\t30\n"
    )
...
def test_demographics_handles_age_column_variants(tmp_path: Path):
    """Age column header is matched case-insensitively."""
    digest = load_digest()
...
def test_demographics_malformed_file_returns_zeros(tmp_path: Path):
    """Unreadable participants.tsv → empty demographics, no exception raised."""
    digest = load_digest()
...
def test_storage_info_returns_none_for_unknown_source(tmp_path: Path):
    """Unknown sources not in STORAGE_CONFIGS → None."""
    digest = load_digest()
...
def test_storage_info_collects_dep_keys_for_other_globals(tmp_path: Path):
    """README, participants.tsv etc. appear in dep_keys (sorted, deduped)."""
    digest = load_digest()
    (tmp_path / "dataset_description.json").write_text("{}")
...
def test_dep_keys_finds_channels_tsv_alongside_recording(tmp_path: Path):
    """channels.tsv alongside a recording appears in dep_keys."""
    digest = load_digest()
...
def test_dep_keys_finds_session_level_sidecar_via_inheritance(tmp_path: Path):
    """Session-level events.json (BIDS inheritance) appears in dep_keys."""
    digest = load_digest()
...
@pytest.mark.parametrize("datatype", ["eeg", "ieeg", "meg", "emg", "nirs"])
...
def test_dep_keys_ignores_another_datatype_sidecar(tmp_path: Path):
    """A record depends on its OWN datatype sidecar, never a co-recorded one.

    At an inherited level the base name carries no datatype, so probing a fixed
    list of every datatype suffix made each record of a co-recorded MEG+EEG
    session depend on the other modality's sidecar too.
    """
    digest = load_digest()
    session_dir = tmp_path / "sub-01" / "ses-01"
    data_dir = session_dir / "eeg"
    data_dir.mkdir(parents=True)
    record = data_dir / "sub-01_ses-01_task-rest_eeg.edf"
    record.touch()
    (session_dir / "sub-01_ses-01_eeg.json").write_text("{}")
    (session_dir / "sub-01_ses-01_meg.json").write_text("{}")
    (session_dir / "sub-01_ses-01_ieeg.json").write_text("{}")

    dep_keys, _, _ = digest._build_dep_keys(
        record, tmp_path, fif_is_split=False, fif_continuations_ok=True
    )
    assert "sub-01/ses-01/sub-01_ses-01_eeg.json" in dep_keys
    assert not [k for k in dep_keys if k.endswith(("_meg.json", "_ieeg.json"))]
...
def test_dep_keys_detects_split_fif_continuation(tmp_path: Path):
    """``<stem>-1.fif`` triggers split-FIF detection."""
    digest = load_digest()
...
def test_dep_keys_broken_fif_continuation_flags_integrity(tmp_path: Path):
    """Broken-symlink FIF continuation sets fif_continuations_ok=False."""
    digest = load_digest()
    meg_dir = tmp_path / "sub-01" / "meg"
    meg_dir.mkdir(parents=True)
    record = meg_dir / "sub-01_task-rest_meg.fif"
...
def test_storage_base_explicit_in_manifest_kept_when_canonical():
    """storage_base matching the source prefix is returned as-is."""
    digest = load_digest()
...
def test_storage_base_explicit_rejected_when_wrong_source():
    """storage_base mismatching the source prefix is rebuilt (openneuro base → nemar source)."""
    digest = load_digest()
...
def test_entities_collects_neuro_subjects_only():
    """Non-neuro files (e.g. anat) are excluded from subjects/tasks counts."""
    digest = load_digest()
    files = [
...
def test_entities_walks_zip_contents():
    """_zip_contents entries inside a ZIP file contribute to subjects/tasks."""
    digest = load_digest()
    files = [
        {
            "path": "data.zip",
...
def test_entities_accepts_plain_string_paths():
    """Files can be plain strings (early manifest schema)."""
    digest = load_digest()
...
def test_entities_standalone_zip_contents_array():
    """Top-level zip_contents array contributes to subjects/tasks."""
    digest = load_digest()
    files = []
...
def test_is_bids_data_zip(filename, expected):
    """_is_bids_data_zip matches BIDS data zip patterns case-insensitively."""
    digest = load_digest()
...
def test_regular_record_accepts_plain_string_file_entry():
    """Plain-string file entries (old manifest shape) are accepted."""
    digest = load_digest()
...
def test_ctf_records_dedup_to_ds_directories():
    """Multiple files inside one .ds dir → one Record per dir."""
    digest = load_digest()
    files = [
