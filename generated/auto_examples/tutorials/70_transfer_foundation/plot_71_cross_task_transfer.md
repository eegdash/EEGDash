...
```none
pretrain losses (RestingState): [0.065, 0.002, 0.001, 0.0, 0.0]
```
...
```none
finetune losses (CCD): [0.564, 0.461, 0.398, 0.352, 0.32]
```
...
```none
finetune=0.667 | scratch=0.617 | chance=0.583 | metric=accuracy | gap=+0.050
```
...
RestingState                    0.150         0.033           0.0
surroundSupp                    0.383         0.033           0.0
symbolSearch                    0.233         0.000           0.0
```
...
| condition           | accuracy |
|---------------------|----------|
| pretrain -> finetune| 0.667   |
| from scratch        | 0.617   |
| chance (majority)   | 0.583   |
{"encoder_weights_path": "plot_71_pretrained_encoder.pt", "pretext_subjects": 8, "target_subjects": 8, "transfer_gap": 0.05, "transfer_matrix_mean_delta": 0.0926}
```
