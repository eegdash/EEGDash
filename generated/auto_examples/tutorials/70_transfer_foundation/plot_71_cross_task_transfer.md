...
```none
pretrain losses (RestingState): [0.05, 0.003, 0.0, 0.0, 0.0]
```
...
```none
finetune losses (CCD): [0.555, 0.469, 0.395, 0.334, 0.313]
```
...
```none
finetune=0.600 | scratch=0.567 | chance=0.650 | metric=accuracy | gap=+0.033
```
...
RestingState                    0.067         0.083         0.167
surroundSupp                    0.450         0.067         0.167
symbolSearch                    0.267        -0.033         0.167
```
...
| condition           | accuracy |
|---------------------|----------|
| pretrain -> finetune| 0.600   |
| from scratch        | 0.567   |
| chance (majority)   | 0.650   |
{"encoder_weights_path": "plot_71_pretrained_encoder.pt", "pretext_subjects": 8, "target_subjects": 8, "transfer_gap": 0.0333, "transfer_matrix_mean_delta": 0.1556}
```
...
**Total running time of the script:** (0 minutes 5.947 seconds)
