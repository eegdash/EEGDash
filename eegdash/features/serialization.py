...
def _load_parallel(path: Path, i: str) -> FeaturesDataset:
    ...
    - ``{i}-feat.safetensors``: Feature values.
    ...
    st_name_pattern = "{}-feat.safetensors"
    st_file_name = st_name_pattern.format(i)
    st_file_path = sub_dir / st_file_name

    tensors = _load_safetensors(str(st_file_path))
    features = pd.DataFrame(tensors)
