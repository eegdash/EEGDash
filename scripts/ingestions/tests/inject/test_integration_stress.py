...
def test_stress_record_with_all_c6_1_fields_round_trips():
    """BIDS-spec fields survive the Gateway → MongoDB → read-back round-trip."""
...
def test_stress_inject_to_invalid_database_rejected():
    """Database name not in valid_databases → 400 or 404."""
...
def test_stress_bulk_insert_100_datasets():
    """100-dataset bulk insert completes under 10 s."""
    inject_mod = load_inject()
...
def test_stress_parallel_inject_no_race_on_distinct_ids():
    """4 parallel inject_datasets calls with distinct IDs all succeed."""
...
def test_stress_parallel_upsert_same_record_no_duplicate():
    """4 parallel upserts on the same composite key produce exactly 1 record (upsert idempotency)."""
...
def test_stress_dataset_with_unicode_name():
    """Unicode name (Greek, Chinese, emoji) survives round-trip."""
    inject_mod = load_inject()
...
def test_stress_dataset_with_very_long_authors_list():
    """50-author list is not truncated by the Gateway."""
    inject_mod = load_inject()
...
def test_stress_record_with_large_inline_sidecar():
    """~100 KB sidecar_inline blob survives the round-trip."""
    inject_mod = load_inject()
...
def test_stress_inject_record_latency_under_500ms():
    """Single-record upsert completes under 2 s."""
    inject_mod = load_inject()
