...
```none
pretrain losses (RestingState): [0.06, 0.008, 0.0, 0.001, 0.0]
```
...
```none
finetune losses (CCD): [0.59, 0.496, 0.418, 0.358, 0.313]
```
...
```none
finetune=0.550 | scratch=0.500 | chance=0.567 | metric=accuracy | gap=+0.050
```
...
RestingState                    0.267         0.050         0.000
surroundSupp                    0.483         0.050         0.000
symbolSearch                    0.367        -0.033        -0.017
```
...
| from scratch        | 0.500   |
| chance (majority)   | 0.567   |
{"encoder_weights_path": "plot_71_pretrained_encoder.pt", "pretext_subjects": 8, "target_subjects": 8, "transfer_gap": 0.05, "transfer_matrix_mean_delta": 0.1296}
```
