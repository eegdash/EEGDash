...
# Marker shapes cycle for up to ~7 cross-subject folds; three are typical.
...
def _draw_pca_panel(
    ...
    *,
    ...
    # in the same coordinates as the scattered points.
...
def _draw_accuracy_panel(
    ...
    *,
    ...
    ax.set_xticklabels(
        [f"fold {i + 1}\nsub-{sid}" for i, sid in enumerate(held_out_subjects)],
        fontsize=8.4,
        color=EEGDASH_INK,
    )
    ...
    ax.text(
        0.0,
        1.06,
        f"Cross-subject CV: {mean_acc:.2f} +/- {std_acc:.2f}",
        transform=ax.transAxes,
        fontsize=10.5,
        fontweight="bold",
        color=EEGDASH_INK,
...
def _draw_confusion_panel(
    ...
    *,
    ...
    ax.text(
        0.0,
        1.06,
        "Held-out fold: confusion matrix",
        transform=ax.transAxes,
        fontsize=10.5,
        fontweight="bold",
        color=EEGDASH_INK,
    ...
    # can still tell whether 0.55 came from 11/20 or 550/1000.
...
def draw_baseline_diagnostic(
    *,
    ...
    """Render the three-panel baseline diagnostic figure.

    Parameters
    ----------
    X_features : numpy.ndarray
        ``(n_windows, n_features)`` band-power feature matrix.
    y_classes : numpy.ndarray
        ``(n_windows,)`` integer class labels (0 / 1).
    ...
    class_names : sequence of str
        Display labels for the two classes (e.g. ``("famous",
        "scrambled")``).
    subjects : sequence of str
        All subjects in the experiment (used in the subtitle).
    held_out_subjects : sequence of str
        Subject id held out on each fold (one per ``fold_accuracies``
        bar).
    chance_level : float
        Chance accuracy reported by ``majority_baseline`` on the test
        fold.
    plot_id : str
    ...
    # Per-fold subject counts (each fold trains on the rest, tests on
    # one held-out subject). The total is shown alongside so the
    # subtitle reflects both the per-fold split and the experiment-level
    # subject pool.
    n_total = len(subjects)
    n_test_per_fold = max(1, len(held_out_subjects) // max(1, len(fold_accuracies)))
    n_train_per_fold = max(1, n_total - n_test_per_fold)
    n_features = X_features.shape[1]
    n_folds = len(fold_accuracies)
    class_pair = " vs ".join(class_names)
    subtitle = (
        f"ds002718 {class_pair} | per-fold n_train_subjects="
    ...
    style_figure(
        fig,
        title=(f"Does a linear band-power baseline beat chance on {class_pair}?"),
        subtitle=subtitle,
        source=source,
        grid_axis="y",
    )
...
__all__ = ["draw_baseline_diagnostic"]
