"""End-to-end pipeline smoke test: stages 3 → 4 → 5 against snapshot fixtures."""
...
def _python_run(script: str, *args: str) -> subprocess.CompletedProcess:
    """Run an ingest script as a subprocess and return the completed result."""
...
def test_stage4_strict_mode_surfaces_unknown_source_warning():
    """``--strict`` promotes the "Unknown source" warning to an error (exit 1)."""
    result = _python_run(
        "4_validate_output.py",
        "--input",
        str(_SNAPSHOT_OUTPUTS),
...
def test_stage5_dry_run_accepts_snapshot():
    """Stage 5 dry-run accepts the snapshot and reports the expected counts."""
    result = _python_run(
        "5_inject.py",
        "--input",
        str(_SNAPSHOT_OUTPUTS),
...
def test_pipeline_contract_3_to_4_to_5():
    """Stage 3's 4 JSON output files per dataset exist and are valid JSON."""
