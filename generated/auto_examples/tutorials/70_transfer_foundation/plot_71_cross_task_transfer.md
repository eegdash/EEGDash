...
```none
pretrain losses (RestingState): [0.041, 0.002, 0.001, 0.001, 0.0]
```
...
```none
finetune losses (CCD): [0.594, 0.539, 0.419, 0.355, 0.324]
```
...
```none
finetune=0.617 | scratch=0.667 | chance=0.550 | metric=accuracy | gap=-0.050
```
...
RestingState                    0.133         0.067           0.0
surroundSupp                    0.483         0.117           0.0
symbolSearch                    0.350        -0.083           0.0
```
...
| condition           | accuracy |
|---------------------|----------|
| pretrain -> finetune| 0.617   |
| from scratch        | 0.667   |
| chance (majority)   | 0.550   |
{"encoder_weights_path": "plot_71_pretrained_encoder.pt", "pretext_subjects": 8, "target_subjects": 8, "transfer_gap": -0.05, "transfer_matrix_mean_delta": 0.1185}
```
