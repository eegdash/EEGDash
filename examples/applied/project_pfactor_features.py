...
file runs independently: it predicts the observed p-factor score from one
...
# ------------------------------------------------------------------
# ``p_factor`` is a recorded participant-level research score. Inspect the
# printed values before conversion; a missing score must not be replaced by a
# label derived from EEG power or array order. This project predicts that
# particular released score and does not interpret it as a diagnosis.
#
...
y = pd.to_numeric(participants["p_factor"], errors="raise").to_numpy(dtype=float)
...
# measured in the released p-factor scale. Lower is better. Compare the model's
