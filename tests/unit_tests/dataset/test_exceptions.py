...
def test_eegdash_error_basics():
    """Test EEGDashError is an Exception with message preservation."""
    assert issubclass(EEGDashError, Exception)
    with pytest.raises(EEGDashError):
        raise EEGDashError("Test error message")
    assert str(EEGDashError("Test error message")) == "Test error message"
...
@pytest.mark.parametrize(
    "method,level", [("log_error", "error"), ("log_warning", "warning")]
)
def test_log_issues(method, level, sample_record):
    """Test log_error/log_warning with full record."""
    error = DataIntegrityError(
        message="Test",
        record=sample_record,
        issues=sample_record["_data_integrity_issues"],
    ...
    with patch("eegdash.dataset.exceptions.logger") as mock_logger:
        getattr(error, method)()
        assert getattr(mock_logger, level).call_count == 3  # Main + 2 issues
...
def test_log_issues_minimal(method, level, minimal_record):
    """Test log_error/log_warning with minimal record."""
    error = DataIntegrityError(
        message="Test",
        record=minimal_record,
        issues=["One issue"],
    ...
    with patch("eegdash.dataset.exceptions.logger") as mock_logger:
        getattr(error, method)()
        assert getattr(mock_logger, level).call_count == 2  # Main + 1 issue
...
def test_log_issues_with_string_contact(method, level, sample_record):
    """Test log_error/log_warning when contact_info is a string."""
    error = DataIntegrityError(
        message="Test",
        record=sample_record,
        issues=["Issue"],
    ...
    with patch("eegdash.dataset.exceptions.logger") as mock_logger:
        getattr(error, method)()
        assert getattr(mock_logger, level).call_count >= 1
